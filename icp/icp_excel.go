...
func FillTaxSheet(file *excelize.File, sheetName string, taxData []TaxObject) error {
	...
	for i, datum := range taxData {
		...
		err = file.SetCellStr(sheetName, fmt.Sprintf("O%d", idx), datum.HsCode)
		...
		err = file.SetCellStr(sheetName, fmt.Sprintf("S%d", idx), datum.DutyParty)
		...
		err = file.SetCellStr(sheetName, fmt.Sprintf("W%d", idx), datum.EoriNo)
		...
		err = file.SetCellStr(sheetName, fmt.Sprintf("Z%d", idx), datum.AddressDetail)
		err = file.SetCellStr(sheetName, fmt.Sprintf("AA%d", idx), datum.PostalCode)
		...
		err = file.SetCellStr(sheetName, fmt.Sprintf("AD%d", idx), datum.Description)
